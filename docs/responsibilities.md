...
- Managing secrets in a safe, secure way, including during production deployments as well as for local development.
...
- We have an Innovation Repo Template that (along with other shared cross-project config) gives guidelines on starting a README and what should be included in it.
...
- Exercise common sense when interacting and communicating with stakeholders
  - Communication with particularly high-level stakeholders (commissioners, for example) needs to be vetted with leadership.
